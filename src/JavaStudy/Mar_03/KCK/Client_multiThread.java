...
public class Client_multiThread extends JFrame {

    char keyChar;

    Socket socket = null;

    public Client_multiThread() {
        //GUI 부분
            ...
            try {
                ...
                while (true) {
                    ...
                    if (inputMessage.equalsIgnoreCase("bye")) {//"bye" 대소문자 관계없이 읽어오면 종료
                        getChat.append(new SimpleDateFormat("[HH:mm:ss] ").format(new Date()) + "서버 : " + inputMessage + "\n");
                        getChat.setCaretPosition(getChat.getDocument().getLength()); //스크롤 맨 아래로 해줌
